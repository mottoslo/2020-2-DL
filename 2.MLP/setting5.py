...
He = tf.contrib.layers.variance_scaling_initializer(factor=2.0, mode='FAN_IN', uniform=False, seed=None, dtype=tf.float32) #He 방법(stddv = sqrt(factor/FAN_IN)인 정규분포)

W1 = tf.get_variable("W1", shape=[784, 200], initializer=He) # 첫 번째 층의 weight matrix
b1 = tf.Variable(tf.random_normal([200]))# 첫 번째 층의 bias
...
W2 = tf.get_variable("W2", shape=[200, 200], initializer=He)
b2 = tf.Variable(tf.random_normal([200]))
L2 = tf.nn.relu(tf.matmul(L1, W2) + b2)
# *hint* tf.nn.dropout으로 dropout 적용
# *hint* tf.nn.l2_loss로 parameter들의 l2_norm 값 누적

W3 = tf.get_variable("W3", shape=[200, 200], initializer=He)
b3 = tf.Variable(tf.random_normal([200]))
L3 = tf.nn.relu(tf.matmul(L2, W3) + b3)
# *hint* tf.nn.l2_loss로 parameter들의 l2_norm 값 누적

W4 = tf.get_variable("W4", shape=[200, 10], initializer=He)
...
learning_rate = 0.001
optimizer = tf.train.AdadeltaOptimizer(learning_rate=learning_rate).minimize(cost) #back propagation
...
# ========================================================================
timestamp = 'adadelta500e'  # setting 바꿀때마다 바꿔주면 폴더가 생김
