...
He = tf.contrib.layers.variance_scaling_initializer(factor=2.0, mode='FAN_IN', uniform=False, seed=None,
                                                    dtype=tf.float32)  # He 방법(stddv = sqrt(factor/FAN_IN)인 정규분포)
keep_prob = tf.placeholder(tf.float32)

W1 = tf.get_variable("W1", shape=[784, 200], initializer=He)  # 첫 번째 층의 weight matrix
W1 = tf.nn.dropout(W1, keep_prob)
...
decayr = 0.01
cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=hypothesis, labels=Y)) + decayr * (
            tf.nn.l2_loss(W1)
            + tf.nn.l2_loss(W2)
            + tf.nn.l2_loss(W3)
            + tf.nn.l2_loss(W4))
...
# ========================================================================
timestamp = 'setting8'  # setting 바꿀때마다 바꿔주면 폴더가 생김
