...
tf.flags.DEFINE_boolean("is_multi_channel", True, "True if multi channel, False if single channel") # default : False
...
tf.flags.DEFINE_boolean("is_non_static", True, "True if non-static")
# Misc Parameters
...
def train(x_train, y_train, word_id_dict, x_dev, y_dev):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs", "text_cnn_multichannel_MR_3,4,5,6"))
            print("Writing to {}\n".format(out_dir))
...
def main(argv=None):
    ...
    time_multichannel = time.time() - start
    print(time_multichannel)
