...
tf.flags.DEFINE_boolean("is_multi_channel", False, "True if multi channel, False if single channel") # default : False
...
def train(x_train, y_train, word_id_dict, x_dev, y_dev):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            out_dir = os.path.abspath(os.path.join(os.path.curdir, "runs", "text_cnn_nonstatic_TREC"))
            print("Writing to {}\n".format(out_dir))
...
def main(argv=None):
    ...
    time_nonstatic = time.time() - start
    print(time_nonstatic)
